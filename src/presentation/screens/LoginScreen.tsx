...
const LoginScreen = ({ navigation }: LoginScreenProps) => {
  ...
  const handleLogin = async () => { if (!email || !password) { setError('Please fill in both fields.'); return; } setIsLoading(true); setError(''); try { await AuthService.signIn({ email, password }); } catch (e: any) { setError('Invalid email or password. Please try again.'); } finally { setIsLoading(false); } };
  ...
  return (
    <SafeAreaView style={styles.flex}>
      <ScrollView contentContainerStyle={styles.container}>
        ...
        <Button mode="contained" onPress={handleLogin} style={styles.button} labelStyle={styles.buttonText} loading={isLoading} disabled={isLoading}>
          Log In
        </Button>
